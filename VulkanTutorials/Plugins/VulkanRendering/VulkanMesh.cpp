...
void VulkanMesh::UploadToGPU(RendererBase* r)  {
	...
	for (int i = 0; i < usedAttributes.size(); ++i) {
		usedBuffers.push_back(vertexBuffer.buffer.get());
	...
	sourceDevice.unmapMemory(stagingBuffer.deviceMem.get());
	...
	{//Now to transfer the vertex data from the staging buffer to the vertex buffer
		...
		cmdBuffer.copyBuffer(stagingBuffer.buffer.get(), vertexBuffer.buffer.get(), copyRegion);
		renderer->SubmitCmdBufferWait(cmdBuffer);
	}

	for (unsigned int i = 0; i < usedAttributes.size(); ++i) {
	...
	vertexInputState = vk::PipelineVertexInputStateCreateInfo({},
		(unsigned int)attributeBindings.size()		, & attributeBindings[0],	//How many buffers this mesh will be taking data from
		(unsigned int)attributeDescriptions.size()	, & attributeDescriptions[0] //how many attributes + attribute info from above
	);
	...
	if (GetIndexCount() > 0) {	//Make the index buffer if there are any!
		...
		vk::CommandBuffer cmdBuffer = renderer->BeginCmdBuffer();
		cmdBuffer.copyBuffer(stagingBuffer.buffer.get(), indexBuffer.buffer.get(), copyRegion);
		renderer->SubmitCmdBufferWait(cmdBuffer);
	}
	if (!debugName.empty()) {
		Vulkan::SetDebugName(sourceDevice, vk::ObjectType::eBuffer, (uint64_t)(VkBuffer)vertexBuffer.buffer.get(), debugName + " vertex attributes");
		if (GetIndexCount() > 0) {
			Vulkan::SetDebugName(sourceDevice, vk::ObjectType::eBuffer, (uint64_t)(VkBuffer)indexBuffer.buffer.get(), debugName + " vertex indices");
		}
	}
}
...
void VulkanMesh::BindToCommandBuffer(vk::CommandBuffer  buffer) const {
	...
	if (GetIndexCount() > 0) {
		buffer.bindIndexBuffer(indexBuffer.buffer.get(), 0, vk::IndexType::eUint32);
	}
}